...
<link rel="stylesheet" type="text/css" href="skin/css/base.css">
	...
	<script type="application/javascript">
        ...
        $(function(){
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/pro/showInfo?mark=1',
                dataType:'json',
            ...
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/emp/getEmpByPid',
                dataType:'json',
                success:function(resultData){
                    $(resultData).each(function(index,item){
                        var option="<option value="+item.eid+">"+item.ename+"--"+item.position.name+"</option>";
                        $("#emp").append(option);
                    });
                },
            });
        ...
        function addayalisys(obj){
            $.ajax({
                ...
                success:function(resultData){
                    ...
                    $.ajax({
                        type:"GET",
                        url:'${pageContext.request.contextPath}/module/getModInfo',
                        data:{"mid":resultData.id},
                        success:function(rd){
							    //$("#mod option").remove();
                                $("#mod option:not(':first')").remove();
                                $(rd).each(function(index,item){
        ...
        function addfunc(obj){
            $.ajax({
                type:'GET',
                url:'${pageContext.request.contextPath}/function/showFunInfo',
				data:{"mid":obj},
                dataType:'json',
                success:function(resultData){
                    console.log(resultData);
                    $("#fun option:not(':first')").remove();
                    $(resultData).each(function(index,item){
